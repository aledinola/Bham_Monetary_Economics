...
| --- | --- | --- | --- |
| [lecture4_extra.m](1_lectures/lecture4_extra.m) | Plain-text live script | **Lecture 4: Consumption, Saving, Interest Rates** | Solves a two-period Huggett-style pure-exchange economy. Illustrates household bond demand, clears a zero-net-supply bond market by price bisection, and compares incomplete markets with full insurance. Studies how an anticipated AI income boom and greater individual earnings risk affect the equilibrium interest rate; includes plots, sensitivity exercises and numerical checks against a closed-form price. |
| [huggett_household.m](1_lectures/huggett_household.m) | Function | **Lecture 4**, helper for `lecture4_extra.m` | Computes optimal bond holdings at a given bond price. Uses `fzero` on the household Euler condition, enforces positive consumption and handles a binding borrowing limit. Returns consumption and Euler-residual diagnostics as well as bond demand. |
| [huggett_equilibrium.m](1_lectures/huggett_equilibrium.m) | Function | **Lecture 4**, helper for `lecture4_extra.m` | Calls `huggett_household`, expands the initial price bracket if needed, and bisects the bond price until excess demand is within tolerance. Returns the equilibrium price, gross return, consumption, residual and iteration history. |
...
- **Live-script format:** these `.m` files contain `%[text]` markup for the Live Editor. The Lecture 4 notebook specifies MATLAB R2025a or newer for this format. Both live scripts state that only base MATLAB is required.
...
The live scripts contain explanatory links to `lecture4_extra.md` and `seminar_class_1.pdf` in the teaching folder. Those documents are not part of this MATLAB-only publication; the links require the local teaching-material copies, but the documents are not required to execute the code.
